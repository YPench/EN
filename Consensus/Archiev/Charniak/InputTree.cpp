...
InputTree::InputTree(istream& is) : parent_(NULL){
  readParse(is);
}
...
void InputTree:: flushConstit(istream& is) {
  int totalOpen = 1;
  for( ; is ; )
    {
      char ch;
...
void InputTree:: readParse(istream& is){
  int pos = 0;
  start_ = pos;
  finish_ = pos;
...
InputTree* InputTree::fixNPBifNecessary(InputTree* nextTree, ECString trm){
...
InputTree* InputTree::newParse(istream& is, int& strt, InputTree* par){
	int strt1 = strt;
	ECString wrd;
	ECString trm;
	ECString ntInf;
...
ECString& InputTree::readNext( istream& is )
{
