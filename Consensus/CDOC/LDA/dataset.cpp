...
int dataset::write_wordmap(string wordmapfile, mapword2id * pword2id) {
	...
	if(LDA_PRINTF_ENABLE){
		DOCCOM::LDA_printf("Cannot open file %s to write!\n", wordmapfile.c_str());
	}
	else{
...
int dataset::read_wordmap(string wordmapfile, mapword2id * pword2id) {
    ...
    if (!fin) {
	if(LDA_PRINTF_ENABLE){
		DOCCOM::LDA_printf("Cannot open file %s to read!\n", wordmapfile.c_str());
	}
	else{
...
int dataset::read_trndata(string dfile, string wordmapfile) {
    ...
    if (M <= 0) {
		if(LDA_PRINTF_ENABLE){
			DOCCOM::LDA_printf("No document available!\n");
		}
		else{
	...
	if (length <= 0) {
		if(LDA_PRINTF_ENABLE){
			DOCCOM::LDA_printf("Invalid (empty) document!\n");
		}
		else{
...
int dataset::read_newdata(string dfile, string wordmapfile) {
    ...
    if (word2id.size() <= 0) {
		if(LDA_PRINTF_ENABLE){
			DOCCOM::LDA_printf("No word map available!\n");
		}
		else{
