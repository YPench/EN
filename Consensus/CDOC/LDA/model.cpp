...
void model::set_default_values() {
    ...
    //dir = "./";
	dir = CDOC_Clustering_Work_Space;
    dfile = "trndocs.dat";
...
int model::load_model(string model_name) {
    ...
    if (!fin) {
		if(LDA_PRINTF_ENABLE){
			DOCCOM::LDA_printf("Cannot open file %d to load model!\n", filename.c_str());
		}
		else{
	...
	if (!pointer) {
		if(LDA_PRINTF_ENABLE){
			DOCCOM::LDA_printf("Invalid word-topic assignment file, check the number of docs!\n");
		}
		else{
	...
	for (j = 0; j < length; j++) {
	    ...
	    if (tok.count_tokens() != 2) {
			if(LDA_PRINTF_ENABLE){
				DOCCOM::LDA_printf("Invalid word-topic assignment line!\n");
			}
			else{
	...
	for (j = 0; (size_t)j < topics.size(); j++) {
	    z[i][j] = topics[j];
	}
    }   
...
int model::save_model_tassign(string filename) {
    ...
    if (!fout) {
		if(LDA_PRINTF_ENABLE){
			DOCCOM::LDA_printf("Cannot open file %s to save!\n", filename.c_str());
		}
		else{
...
int model::init_est() {
    ...
    if (ptrndata->read_trndata(dir + dfile, dir + wordmapfile)) {
		if(LDA_PRINTF_ENABLE){
			DOCCOM::LDA_printf("Fail to read training data!\n");
		}
		else{
...
int model::init_estc() {
    ...
    if (load_model(model_name)) {
		if(LDA_PRINTF_ENABLE){
			DOCCOM::LDA_printf("Fail to load word-topic assignmetn file of the model!\n");
		}
		else{
...
void model::estimate() {
	...
	if(LDA_PRINTF_ENABLE){
		DOCCOM::LDA_printf("Sampling %d iterations!\n", niters);
	}
	else{
	...
	if (savestep > 0) {
		...
		// saving the model
			if(LDA_PRINTF_ENABLE){
				DOCCOM::LDA_printf("Saving the model at iteration %d ...\n", liter);
			}
			else{
    ...
    if(LDA_PRINTF_ENABLE){
		DOCCOM::LDA_printf("Gibbs sampling completed!\n");
		DOCCOM::LDA_printf("Saving the final model!\n");
	}
	else{
...
int model::init_inf() {
	...
	if (pnewdata->read_newdata_withrawstrs(dir + dfile, dir + wordmapfile)) {
		if(LDA_PRINTF_ENABLE){
			DOCCOM::LDA_printf("Fail to read new data!\n");
		}
		else{
...
void model::inference() {
	...
	if(LDA_PRINTF_ENABLE){
		DOCCOM::LDA_printf("Sampling %d iterations for inference!\n", niters);
	}
	else{
    ...
    if(LDA_PRINTF_ENABLE){
		DOCCOM::LDA_printf("Gibbs sampling for inference completed!\n");
		DOCCOM::LDA_printf("Saving the inference outputs!\n");
	}
	else{
