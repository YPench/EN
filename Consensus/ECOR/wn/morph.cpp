...
static int do_init(void)
{
    ...
    for (i = 1; i <= NUMPARTS; i++) {
	sprintf_s(fname, 256, EXCFILE, searchdir, partnames[i]);
	if (fopen_s(&exc_fps[i], fname, "r")) {
...
char *morphstr(char *origstr, int pos)
{
	...
	strcpy_s(str, 256, origstr);//YPench;
	...
	if (pos == VERB && cnt > 1 && (prep = hasprep(str, cnt))) {
		...
		strncpy_s(word, 256, str + st_idx, end_idx - st_idx);
		word[end_idx - st_idx] = '\0';
		if(tmp = morphword(word, pos))
		    strcat_s(searchstr, 256, tmp);
		else
		    strcat_s(searchstr, 256, word);
		strcat_s(searchstr, 256, append);
		st_idx = end_idx + 1;
	    }
	    strcpy_s(word, 256, str + st_idx);//:YPench
	    if(tmp = morphword(word, pos)) 
		strcat_s(searchstr, 256, tmp);
	    else
		strcat_s(searchstr, 256, word);
...
char *morphword(char *word, int pos)
{
	...
	if (strend(word, "ful")) {
	    ...
	    strncat_s(tmpbuf, WORDBUF, word, cnt);
	    end = "ful";
	} else 
...
/* If not in exception list, try applying rules from tables */
    ...
    if (tmpbuf[0] == '\0')
	strcpy_s(tmpbuf, WORDBUF, word);

    offset = offsets[pos];
    cnt = cnts[pos];

    for(i = 0; i < cnt; i++){
	strcpy_s(retval, WORDBUF, wordbase(tmpbuf, (i + offset)));
	if(strcmp(retval, tmpbuf) && is_defined(retval, pos)) {
	    strcat_s(retval, WORDBUF, end);
	    return(retval);
	}
    }
    return(NULL);
...
static char *wordbase(char *word, int ender)
{
    ...
    strcpy_s(copy, WORDBUF, word);
    if(strend(copy,sufx[ender])) {
	pt1=strchr(copy,'\0');
	pt1 -= strlen(sufx[ender]);
	*pt1='\0';
	strcat_s(copy, WORDBUF, addr[ender]);
    }
    return(copy);
}
...
static char *morphprep(char *s)
{
	...
	if (lastwd = morphword(last + 1, NOUN)) {
	    strncpy_s(end, WORDBUF, rest, last - rest + 1);
	    end[last-rest+1] = '\0';
	    strcat_s(end, WORDBUF, lastwd);
	}
    }
    
    strncpy_s(word, WORDBUF, s, rest - s);
    word[rest - s] = '\0';
