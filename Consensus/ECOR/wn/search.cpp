...
extern std::vector<string> synset_v;
...
IndexPtr index_lookup(char *word, int dbase)
{
	...
	sprintf_s(msgbuf, 256, "WordNet library error: %s indexfile not open\n",
		partnames[dbase]);
...
IndexPtr parse_index(long offset, int dbase, char *line) {
    ...
    assert(idx->wd);
    strcpy_s(idx->wd, strlen(ptrtok) + 1, ptrtok);
    
    /* get the part of speech */
    ptrtok=strtok_s(NULL," \n", &context);
    idx->pos = (char*)malloc(strlen(ptrtok) + 1);
    assert(idx->pos);
    strcpy_s(idx->pos, strlen(ptrtok) + 1, ptrtok);
...
SynsetPtr read_synset(int dbase, long boffset, char *word)
{
	...
	sprintf_s(msgbuf, 256, "WordNet library error: %s datafile not open\n",
		partnames[dbase]);
	display_message(msgbuf);
	return(NULL);
    }
...
SynsetPtr parse_synset(FILE *fp, int dbase, char *word)
{
	...
	sprintf_s(msgbuf, 256, "WordNet library error: no synset at location %d\n",
		loc);
	display_message(msgbuf);
	free(synptr);
	return(NULL);
    ...
    assert(synptr->pos);
    strcpy_s(synptr->pos, strlen(ptrtok) + 1, ptrtok);
	...
	strcpy_s(synptr->words[i], strlen(ptrtok) + 1, ptrtok);
	...
	ptrtok = strtok_s(NULL, " \n", &context);
	sscanf_s(ptrtok, "%x", &synptr->lexid[i]);
    }
	...
	sprintf_s(tmpbuf, 10240, "%c:%8.8d", partchars[dbase], synptr->hereiam);
...
static void traceptrs(SynsetPtr synptr, int ptrtyp, int dbase, int depth)
{
	...
	if ((ptrtyp == HYPERPTR && (synptr->ptrtyp[i] == HYPERPTR ||
	    ...
	    case PERTPTR:
		if (dbase == ADV) 
		    sprintf_s(prefix, 40, "Derived from %s ",
			    partnames[synptr->ppos[i]]);
		else
		    sprintf_s(prefix, 40, "Pertains to %s ",
			    partnames[synptr->ppos[i]]);
		break;
	    case ANTPTR:
		if (dbase != ADJ)
		    sprintf_s(prefix, 40, "Antonym of ");
		break;
	    case PPLPTR:
		sprintf_s(prefix, 40, "Participle of verb ");
		break;
	    case INSTANCE:
		sprintf_s(prefix, 40, "INSTANCE OF=> ");
		break;
	    case INSTANCES:
		sprintf_s(prefix, 40, "HAS INSTANCE=> ");
		break;
	    case HASMEMBERPTR:
		sprintf_s(prefix, 40, "   HAS MEMBER: ");
		break;
	    case HASSTUFFPTR:
		sprintf_s(prefix, 40, "   HAS SUBSTANCE: ");
		break;
	    case HASPARTPTR:
		sprintf_s(prefix, 40, "   HAS PART: ");
		break;
	    case ISMEMBERPTR:
		sprintf_s(prefix, 40, "   MEMBER OF: ");
		break;
	    case ISSTUFFPTR:
		sprintf_s(prefix, 40, "   SUBSTANCE OF: ");
		break;
	    case ISPARTPTR:
		sprintf_s(prefix, 40, "   PART OF: ");
		break;
	    default:
		sprintf_s(prefix, 40, "=> ");
		break;
...
static void traceclassif(SynsetPtr synptr, int dbase, int search)
{
	...
	if (((synptr->ptrtyp[i] >= CLASSIF_START) &&
		...
		if (synptr->ptrtyp[i] == CLASSIF_CATEGORY)
		    strcpy_s(head, 60, "TOPIC->(");
		else if (synptr->ptrtyp[i] == CLASSIF_USAGE)
		    strcpy_s(head, 60, "USAGE->(");
		else if (synptr->ptrtyp[i] == CLASSIF_REGIONAL)
		    strcpy_s(head, 60, "REGION->(");
		else if (synptr->ptrtyp[i] == CLASS_CATEGORY)
		    strcpy_s(head, 60, "TOPIC_TERM->(");
		else if (synptr->ptrtyp[i] == CLASS_USAGE)
		    strcpy_s(head, 60, "USAGE_TERM->(");
		else if (synptr->ptrtyp[i] == CLASS_REGIONAL)
		    strcpy_s(head, 60, "REGION_TERM->(");

		strcat_s(head, 60, partnames[synptr->ppos[i]]);
		strcat_s(head, 60, ") ");
...
char *findtheinfo(char *searchstr, int dbase, int ptrtyp, int whichsense)
{
	...
	//}
	//else if (overflag){
	//	sprintf_s(searchbuffer,SEARCHBUF, "Search too large.  Narrow search and try again...\n");
	//}
...
SynsetPtr traceptrs_ds(SynsetPtr synptr, int ptrtyp, int dbase, int depth)
{
	...
	for (i = 0; i < synptr->ptrcount; i++)
		...
		strcpy_s(synptr->headword, strlen(cursyn->words[0]) + 1, cursyn->words[0]);
...
"                                                                                                   \n");
	...
	for (sense = 0; sense < idx->off_cnt; sense++) {
		...
		if (idx->tagged_cnt != -1 &&
		  ...
		  sprintf_s(tmpbuf, 10240, "%d. (%d) ",
			  sense + 1, GetTagcnt(idx, sense + 1));
		} else {
		  sprintf_s(tmpbuf, 10240, "%d. ", sense + 1);
		}
	...
	if (i > 0) {
	    ...
	    if (idx->tagged_cnt > 0)
		sprintf_s(tmpbuf + strlen(tmpbuf), 10240, 
...
static int getsearchsense(SynsetPtr synptr, int whichword)
{
	...
	strcpy_s(wdbuf, 256, synptr->words[whichword - 1]);
    strsubst(wdbuf, ' ', '_');
    strtolower(wdbuf);
...
static void printsynset(char *head, SynsetPtr synptr, char *tail, int definition, int wdnum, int antflag, int markerflag)
{
   ...
   // if (offsetflag)		/* print synset offset */
    ...
    if(definition && dflag && synptr->defn) {
		strcat_s(tbuf, 3072, " -- ");
		strcat_s(tbuf, 3072, synptr->defn);
    }

    strcat_s(tbuf, 3072, tail);
    printbuffer(tbuf);
}
...
static void printantsynset(SynsetPtr synptr, char *tail, int anttype, int definition)
{
    ...
    if (offsetflag)
	sprintf_s(tbuf, 3072, "{%8.8d} ", synptr->hereiam);
    if (fileinfoflag) {
	sprintf_s(tbuf + strlen(tbuf), 3072, "<%s> ", lexfiles[synptr->fnum]);
	prlexid = 1;
    } else
    ...
    strcat_s(tbuf, 3072, "INDIRECT (VIA ");
    for(i = 0, wdcnt = synptr->wcount; i < wdcnt; i++) {
	if (first) {
...
static void catword(char *buf, SynsetPtr synptr, int wdnum, int adjmarker, int antflag)
{
    ...
    strcpy_s(wdbuf, 256, synptr->words[wdnum]);
    strcat_s(buf, 3072, deadjify(wdbuf));//:YPench
	synset_v.push_back(deadjify(wdbuf));
    /* Print additional lexicographer information and WordNet sense
       number as indicated by flags */
	
	//if (prlexid && (synptr->lexid[wdnum] != 0)){
	//	sprintf_s(buf + strlen(buf), 3072, "%d", synptr->lexid[wdnum]);
	//}
	//if (wnsnsflag){
	//	sprintf_s(buf + strlen(buf), 3072, "#%d", synptr->wnsns[wdnum]);
	//}
	...
	if (getpos(synptr->pos) == ADJ) {
		if (adjmarker == PRINT_MARKER){
			strcat_s(buf, 3072, markers[adj_marker]);
		}
		if (antflag == PRINT_ANTS){
			strcat_s(buf, 3072, printant(ADJ, synptr, wdnum + 1, vs, ""));
		}
    }
}

static char *printant(int dbase, SynsetPtr synptr, int wdnum, char *ctemplate, char *tail)
{
    int i, j, wdoff;
    SynsetPtr psynptr;
    char tbuf[WORDBUF];
	...
	if (synptr->ptrtyp[i] == ANTPTR && synptr->pfrm[i] == wdnum) {
		...
		if (psynptr->ptrtyp[j] == ANTPTR &&
		    ...
		    strcpy_s(wdbuf, 256, psynptr->words[wdoff]);
		    strcpy_s(tbuf, 256, deadjify(wdbuf));

		    /* Print additional lexicographer information and
		       WordNet sense number as indicated by flags */
	
		    if (prlexid && (psynptr->lexid[wdoff] != 0))
			sprintf_s(tbuf + strlen(tbuf), 256, "%d",
				psynptr->lexid[wdoff]);
		    if (wnsnsflag)
			sprintf_s(tbuf + strlen(tbuf), 256, "#%d",
				psynptr->wnsns[wdoff]);
		    if (!first)
			strcat_s(retbuf, 3072, tail);
		    else
			first = 0;
		    sprintf_s(retbuf + strlen(retbuf), 3072, ctemplate, tbuf);
		}
	    }
	    free_synset(psynptr);
	}
...
static void printbuffer(char *string)
{
	...
	strcat_s(searchbuffer, SEARCHBUF, string);
}
